...
df_injuries = pd.read_csv(r'.\data\WHOMortalityDatabase_Deaths_sex_age_a_country_area_year-{filter_item_clustered_column_chart_5_key.selected_filter.valueLabels}_31st August 2025 18_48.csv', skiprows=6, index_col=False)
df_injuries['cause']="Injury"
df_comm = pd.read_csv(r'.\data\WHOMortalityDatabase_Deaths_sex_age_a_country_area_year-Communicable, maternal, perinatal and nutritional conditions_31st August 2025 18_45.csv', skiprows=6, index_col=False)
df_comm['cause']="Communicable"
df_noncomm = pd.read_csv(r'.\data\WHOMortalityDatabase_Deaths_sex_age_a_country_area_year-Noncommunicable diseases_31st August 2025 18_46.csv', skiprows=6, index_col=False)
...
df_concat = df_concat.dropna(thresh=df_concat.shape[1]-1)
...
df_concat.to_csv(r'.\data\output\data.csv')



# Clean age group strings to a single number
def age_to_number(age_str):
    """
    Extracts the first number from an age group string (e.g., '1-4' -> 1).
    """
    if pd.isna(age_str):
        return None  # Use None instead of 0 for clarity
    age_str = str(age_str)
...
# Pivot the data
# Define a function to map age groups to broader bins
def map_age_robust(age_str):

    # Use the age_to_number function to get a numeric value from the string
...
# mapping broad year into groups of decades
def year_map(year):
    int_year=int(year)
    if int_year <= 1970:
        return '1961-1970'
...
# Use the more robust mapping function
df_concat['Age Group Broad'] = df_concat['Age Group'].apply(map_age_robust)

#mapping the year to borad year
...
plt.tight_layout()
plt.savefig(r".\data\output\Cause_of_death_by_Broad_Age_group.png")
plt.show()
